...
class AssetTab(QWidget, MessageBoxMixin, Logger):
    ...
    def __init__(self, window: "ElectrumWindow"):
        ...
        if self.wallet.is_watching_only():
            self.reissue_asset_tab = QLabel(
                _("Watch only wallets cannot reissue assets")
            )
        ...
        menu.addConfig(
            _("Download IPFS"),
            window.config.cv.DOWNLOAD_IPFS,
            callback=self.view_asset_tab.metadata_viewer.metadata_info.ipfs_viewer.update_visibility,
        )
        menu.addConfig(
            _("Display Downloaded IPFS"),
            window.config.cv.SHOW_IPFS,
            callback=self.view_asset_tab.metadata_viewer.metadata_info.ipfs_viewer.update_visibility,
        )
        ...
        menu.addConfig(
            _("Control Asset Address"),
            window.config.cv.SHOW_CREATE_ASSET_PAY_TO,
            callback=maybe_update_manage_tabs,
        )
        menu.addConfig(
            _("Display Unconfirmed Information"),
            window.config.cv.HANDLE_UNCONFIRMED_METADATA,
            callback=self.update,
        )
        menu.addConfig(
            _("Show Metadata Sources"),
            window.config.cv.SHOW_METADATA_SOURCE,
            callback=self.view_asset_tab.update,
        )
        menu.addConfig(
            _("Lookup IPFS using all gateways"),
            window.config.cv.ROUND_ROBIN_ALL_KNOWN_IPFS_GATEWAYS,
        )
        ...
        menu.addConfig(
            _("Display IPFS as base32 CIDv1"),
            window.config.cv.SHOW_IPFS_AS_BASE32_CIDV1,
            callback=update_this_and_broadcast_tab,
        )
        menu.addConfig(
            _("Verify Transitory Asset Data"),
            window.config.cv.VERIFY_TRANSITORY_ASSET_DATA,
        )

        self.is_hardware = (
            self.wallet.keystore and self.wallet.keystore.get_type_text()[:2] == "hw"
        )
        ...
        if not self.is_hardware:
            toolbar.addWidget(
                QLabel(_("Select a tab below to view, create, and manage your assets"))
            )
        toolbar.addStretch()
        ...
        if self.is_hardware:
            ...
            tabs.addTab(
                self.create_asset_tab, read_QIcon("unconfirmed.png"), _("Create")
            )
            ...
            tabs.addTab(
                self.broadcast_tab, read_QIcon("broadcast_send.png"), _("Broadcast")
            )
    ...
    def display_asset(self, asset: str):
        self.view_asset_tab.asset_list.select_asset(asset)
        if not self.is_hardware:
            self.tabs.setCurrentIndex(self.tabs.indexOf(self.view_asset_tab))
