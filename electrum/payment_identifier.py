...
class PaymentIdentifier(Logger):
    ...
    def __init__(self, wallet: Optional['Abstract_Wallet'], text: str, *, asset=None):
    ...
    def is_onchain(self):
        if self._type in [PaymentIdentifierType.SPK, PaymentIdentifierType.MULTILINE, PaymentIdentifierType.BIP70,
                          PaymentIdentifierType.OPENALIAS, PaymentIdentifierType.BIP21]:
            return True
    ...
    def get_asset(self):
        if self.spk or self.multiline_outputs:
            return self.default_asset
        elif self.bip70:
            outputs = self.bip70_data.get_outputs()
            _sentiel = object()
            asset = _sentiel
            assert outputs
            for output in outputs:
                ...
                if output.asset != asset:
                    raise Exception('asset mismatches in bip70')
            return asset
        elif self.bip21:
            return self.bip21.get('asset', None)
        else:
